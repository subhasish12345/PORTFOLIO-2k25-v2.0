              ...
              <div className="flex justify-center mt-4 gap-2 overflow-x-auto pb-2">
                {galleryImages[selectedGallery as keyof typeof galleryImages].map((image, index) => (
                  <button
                    ...
                    className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all duration-200 ${index === currentImageIndex ? 'border-purple-400' : 'border-transparent'
                      }`}
                  >
                    <img
                      src={image}
